...
?>
<form id="myForm" action="students.php" onchange="myFunction()">
...
<?php
                            ...
                            $query='SELECT * FROM students LIMIT ' . $this_page_first_result . ',' .  $results_per_page;
                            $query_run = mysqli_query($con,$query);

                   $query2= "SELECT MAX(id) AS ids FROM students" ;
                    $result_query= mysqli_query($con,$query2);
    ...
    <div class="modal-dialog">
        ...
        <form id="saveStudent" method="post" enctype="multipart/form-data"> 
            <div class="modal-body">
                ...
                <div class="mb-3">
                <input type="file" name="file" id="file" class="fileToUpload form-control" ></input><br>
    <input type="text" placeholder="File name" id="filename" name="filename" value = "<?php echo $output +1 ;?>" class="form-control"/>
                </div>
                <div class="mb-3">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" onclick="uploadfile()">Save Student</button>
            </div>
        </form>
        </div>
    </div>
...
<div class="modal fade" id="studentEditModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        ...
        <form id="updateStudent">
            ...
            <div class="mb-3">
                <input type="file" name="fileup" id="fileup" class="fileToUp form-control" ></input><br>
                <input type="text" placeholder="File name" id="filen" name="filen" value = "" class="form-control"/>
            </div>
                <div class="mb-3">
                    <label for="">Name</label>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-primary" onclick="updatefile()" >Update Student</button>
            </div>
        </form>
        </div>
    </div>
...
<div class="container mt-4">
                            ...
                            if($rowcount > 0)
                            {
                                foreach($query_run as $student)
                                {
                                    ...
                                    <tr>
                                        ...
                                        {
                                            ?><td><img src="http://localhost/0.9/upload/<?=$student['id'] ?>.png" width="30px" height="30px"></td><?php 
                                        }
                                        ?>
                                        <td><?= $student['id'] ?></td>
                                        <td><?= $student['name'] ?></td>
    ...
    <script>
        ...
        $(document).on('click', '.editStudentBtn', function () {
            ...
            var filename = $('#filen').val();                    //To save file with this name
            var file_data = $('.fileToUp').prop('files')[0];    //Fetch the file
            var form_data = new FormData();
            form_data.append("fileup",file_data);
            form_data.append("filen",filename);
...
for ($page=1;$page<=$number_of_pages;$page++) {
  echo '<button id="d"> <a href="students.php?page=' . $page . '&select=' . $results_per_page . ' ">' . $page . '</a></button> ';
}
?>
<?php
}
...
{
    echo "access denied";
    header("Refresh:2;url= http://localhost/0.9/");
}
?>
</body>
</html>
